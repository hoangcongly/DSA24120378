...
void removeAfter(List *&L, int val)
{
    if(L->p_head==NULL|| L->p_head->p_next==NULL) return;
    NODE*cur=L->p_head;
    while(cur!=NULL&& cur->p_next!=NULL&& cur->key!=val)
    {
        cur=cur->p_next;
    }
   // 3 6 3 2 5 1 NULL 
    if(cur->p_next!=NULL&& cur->p_next->p_next!=NULL)
    {
        NODE*temp=cur->p_next;
...
void removePos(List *&L, int data, int pos)
{
    ...
    if(cur!=NULL&& cur->key==data)
    {
        prev->p_next=cur->p_next;
        if(cur==L->p_tail) L->p_tail=prev;
        delete cur;
        return;
    }
}
...
List *reverseList(List *L)
{
    List*Lre=createList(NULL);
    NODE*cur=L->p_head;
    while(cur!=NULL) 
    {
        NODE*newNode=createNode(cur->key);
        newNode->p_next=Lre->p_head;
        Lre->p_head=newNode;
        if(L->p_head==NULL)
        {
            L->p_head=L->p_tail=newNode;
