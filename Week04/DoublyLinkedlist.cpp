...
void removeBefore(List *&L, int val)
{
    ...
    // 1 2 3 4 5 6 3 1 nullptr
    if(cur==NULL|| cur==L->p_head) return;
    if(prevcur->p_prev!=NULL)
    {
        NODE*temp=prevcur;
        prevcur->p_prev->p_next=cur;
        cur->p_prev= prevcur->p_prev;
        delete temp;
    }
    else
    {
...
void removeAfter(List *&L, int val)
{
    ...
    if( cur==NULL|| cur->p_next==NULL) return;
    if(cur->p_next->p_next!=NULL)
    { 
        NODE*temp=cur->p_next;
        cur->p_next=cur->p_next->p_next;
        cur->p_next->p_next->p_prev=cur;
        delete temp;
        return;
    }
    else
...
List *reverseList(List *L)
{
    List*Listnew=createList(NULL);
    NODE*cur=L->p_head;
    while(cur)
    {
        NODE*newNode=createNode(cur->key);
        newNode->p_next=Listnew->p_head;
        if(Listnew->p_head!=NULL) Listnew->p_head->p_prev=newNode;
        else Listnew->p_tail=newNode;
        Listnew->p_head=newNode;
        cur=cur->p_next;
    }
    return Listnew;
}
...
void removeDuplicate(List *&L)
{
    ...
    while (cur)
    {
        ...
        while (curnext)
        {
            if (curnext->key == cur->key)
            {
                NODE* temp = curnext;
                prev->p_next = curnext->p_next;
                if (curnext->p_next != NULL)
                {
                    curnext->p_next->p_prev = prev;
                }
                else
                {
                    L->p_tail = prev;
                }
                curnext = curnext->p_next;
                delete temp;
            }
            else
            {
